...
import { ShowContextMenuInputType } from '../../Menu/CustomContextMenu'
...
const FileItem: React.FC<FileInfoProps> = ({
  ...
  const handleFocusDirOrFile = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (isDirectory) showContextMenu(event, 'DirectoryItem', { file })
    else showContextMenu(event, 'FileItem', { file })
  }
