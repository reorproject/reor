    ...
    <div className="relative bottom-5 flex w-full max-w-3xl">
      <div className="w-full rounded-lg border-solid border-neutral-700 p-3">
        <div className="relative flex h-full pr-8">
          <textarea
            onKeyDown={(e) => {
              if (userTextFieldInput && !loadingResponse && !e.shiftKey && e.key === 'Enter') {
                e.preventDefault()
                handleSubmitNewMessage()
              }
            }}
            ...
            disabled={loadingResponse} // Disable input when loading response
          />
          <div className="absolute right-3 top-1/2 -translate-y-1/2">
            <PiPaperPlaneRight
              color={userTextFieldInput && !loadingResponse ? 'white' : 'gray'}
              onClick={!loadingResponse && userTextFieldInput ? handleSubmitNewMessage : undefined}
              className={!loadingResponse && userTextFieldInput ? 'cursor-pointer' : ''}
            />
          </div>
        </div>
      </div>
